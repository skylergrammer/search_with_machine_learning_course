...
def create_rescore_ltr_query(user_query: str, query_obj, click_prior_query: str, ltr_model_name: str,
    ...
    query_obj["rescore"] = rescore_params
...
def create_feature_log_query(query, doc_ids, click_prior_query, featureset_name, ltr_store_name, size=200, terms_field="_id"):
    ...
    # Filter only doc ids in specified term field
    filter_term_field = {
        "terms": {
            terms_field: doc_ids
        }
    ...
    # Turns on feature logging so that we get weights back for our features
    q_ext = {
        "ltr_log": {
            "log_specs": {
                "name": "log_entry",
    ...
    query_obj = {
        ...
        "query": {
            "bool": {
                "filter": [filter_term_field, filter_sltr]
            }
        },
        "ext": q_ext
    }
